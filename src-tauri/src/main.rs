...
fn snapshot(scene: &model::Scene) -> SceneSnapshot {
    SceneSnapshot { revision: scene.revision(), objects: scene.objects().cloned().collect() }
}
...
fn save_project(path: String, state: tauri::State<'_, SceneState>) -> Result<(), String> {
    let scene = state.0.lock().map_err(|_| "scene state is unavailable".to_owned())?;
    archive::write_archive(Path::new(&path), &scene.objects().cloned().collect::<Vec<_>>())
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            inspect_gds_file, import_gds, scene_snapshot, update_object_display, save_project, load_project,
        ])
